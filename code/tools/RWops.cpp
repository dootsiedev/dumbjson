...
public:
	...
	size_t read(void* ptr, size_t size, size_t nmemb) override
	{
		...
		if(bytes_read != size && ferror(fp) != 0)
		{
			...
			// that is too painful.
			serrf("Error reading from datastream: `%s` (return: %zu)\n", stream_info, bytes_read);
		}
		return bytes_read;
	}
	...
	int tell() override
	{
		...
		return static_cast<int>(ftell(fp));
	}
	bool good() override
	{
		return ferror(fp) == 0;
...
public:
	...
	size_t read(void* ptr, size_t size, size_t nmemb) override
	{
		...
		//(new SDL2 feature): SDL_GetErrorMsg is better because it fixes a dangerous race condition
		// to a safe race condition, but still gives the wrong result without a mutex, so just stick
		// to one thread, ok? char buffer[1024]; const char* error = SDL_GetErrorMsg(buffer,
		// sizeof(buffer)); char buffer[1024];
	...
	int seek(int offset, int whence) override
	{
		int out = static_cast<int>(SDL_RWseek(sdl_ops, offset, whence));
		if(out < 0)
		{
			// char buffer[1024];
			const char* error = SDL_GetError(); // SDL_GetErrorMsg(buffer, sizeof(buffer));
	...
	int tell() override
	{
		int out = static_cast<int>(SDL_RWtell(sdl_ops));
		if(out < 0)
		{
			// char buffer[1024];
			const char* error = SDL_GetError(); // SDL_GetErrorMsg(buffer, sizeof(buffer));
...
Unique_RWops RWops_OpenFS(const char* path, const char* mode)
{
...
Unique_RWops RWops_FromFP(FILE* fp, const char* name)
{
	return std::make_unique<RWops_Stdio_AutoClose>(fp, name);
}
Unique_RWops RWops_FromFP_NoClose(FILE* fp, const char* name)
{
	return std::make_unique<RWops_Stdio_NoClose>(fp, name);
}
Unique_RWops RWops_FromMemory(char* memory, size_t size, const char* name)
{
	SDL_RWops* sdl_rwop = SDL_RWFromMem(memory, static_cast<int>(size));
	if(sdl_rwop == NULL)
	{
...
Unique_RWops RWops_FromMemory_ReadOnly(char* memory, size_t size, const char* name)
{
	SDL_RWops* sdl_rwop = SDL_RWFromConstMem(memory, static_cast<int>(size));
	if(sdl_rwop == NULL)
	{
		const char* error = SDL_GetError();
		serrf("Failed to open: `%s`, reason: %s\n", name, error);
